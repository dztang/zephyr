...
.. note:: The :c:macro:`SMF_CREATE_STATE` macro always takes five arguments
	(entry action, run action, exit action, parent state, initial
	transition) even for a flat state machine with no parent or initial
	transition, or for hierarchical state machines with
	:kconfig:option:`CONFIG_SMF_INITIAL_TRANSITION` not enabled. Unused
...
.. note:: While the state machine is running, :c:func:`smf_set_state` should
   only be called from the Run function. Calling :c:func:`smf_set_state` from
   Entry and Exit functions will generate a warning in the log and no
