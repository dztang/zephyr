					   ...
					   const struct mipi_dbi_config *dbi_config)
{
	...
	flexioMcuLcdConfig.baudRate_Bps = dbi_config->config.frequency * config->data_bus_width;
...
static int flexio_lcdif_init(const struct device *dev)
{
	...
	stream->dma_cfg.source_data_size = config->data_bus_width / 8;
...
#define MCUX_FLEXIO_LCDIF_DEVICE_INIT(n)						\
	...
	struct mcux_flexio_lcdif_config mcux_flexio_lcdif_config_##n = {		\
		...
		.data_bus_width = DT_INST_PROP(n, data_bus_width),			\
