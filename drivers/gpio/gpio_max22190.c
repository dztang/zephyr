...
typedef struct _max22190_filter {
...
static int max22190_reg_transceive(const struct device *dev, uint8_t addr, uint8_t val, uint8_t rw)
{
	...
	local_tx_buff[0] = FIELD_PREP(MAX22190_ADDR_MASK, addr) |
				FIELD_PREP(MAX22190_RW_MASK, rw & 0x1);
	local_tx_buff[1] = val;
...
static void max22190_fault_check(const struct device *dev)
{
	...
	if (_fault1) {
		memcpy(&data->fault1, &_fault1, 1);

		/* FAULT1_EN */
		uint8_t _fault1_en = max22190_reg_transceive(dev, MAX22190_FAULT1_EN_REG, 0,
								MAX22190_READ);

		memcpy(&data->fault1_en, &_fault1_en, 1);

		PRINT_ERR_BIT(data->fault1.max22190_CRC,
				data->fault1_en.max22190_CRCE);
		PRINT_ERR_BIT(data->fault1.max22190_POR,
				data->fault1_en.max22190_PORE);
		PRINT_ERR_BIT(data->fault1.max22190_FAULT2,
				data->fault1_en.max22190_FAULT2E);
		PRINT_ERR_BIT(data->fault1.max22190_ALRMT2,
				data->fault1_en.max22190_ALRMT2E);
		PRINT_ERR_BIT(data->fault1.max22190_ALRMT1,
				data->fault1_en.max22190_ALRMT1E);
		PRINT_ERR_BIT(data->fault1.max22190_24VL,
				data->fault1_en.max22190_24VLE);
		PRINT_ERR_BIT(data->fault1.max22190_24VM,
				data->fault1_en.max22190_24VME);
		PRINT_ERR_BIT(data->fault1.max22190_WBG,
				data->fault1_en.max22190_WBGE);

		if (data->fault1.max22190_WBG & data->fault1_en.max22190_WBGE) {
			uint8_t wb_val = max22190_reg_transceive(dev, MAX22190_WB_REG, 0,
										MAX22190_READ);
			max22190_update_wb_stat(dev, (uint8_t)wb_val);
		}

		if (data->fault1.max22190_FAULT2) {
			/* FAULT2 */
			uint8_t _fault2 = max22190_reg_transceive(dev, MAX22190_FAULT2_REG, 0,
								MAX22190_READ);

			memcpy(&data->fault2, &_fault2, 1);

			/* FAULT2_EN */
			uint8_t _fault2_en = max22190_reg_transceive(dev, MAX22190_FAULT2_EN_REG, 0,
								MAX22190_READ);

			memcpy(&data->fault2_en, &_fault2_en, 1);
			PRINT_ERR_BIT(data->fault2.max22190_RFWBS,
					data->fault2_en.max22190_RFWBSE);
			PRINT_ERR_BIT(data->fault2.max22190_RFWBO,
					data->fault2_en.max22190_RFWBOE);
			PRINT_ERR_BIT(data->fault2.max22190_RFDIS,
					data->fault2_en.max22190_RFDISE);
			PRINT_ERR_BIT(data->fault2.max22190_RFDIO,
					data->fault2_en.max22190_RFDIOE);
			PRINT_ERR_BIT(data->fault2.max22190_OTSHDN,
					data->fault2_en.max22190_OTSHDNE);
			PRINT_ERR_BIT(data->fault2.max22190_FAULT8CK,
					data->fault2_en.max22190_FAULT8CKE);
		}
	}
}
...
static void max22190_filter_set(const struct device *dev)
{
	...
	for (int ch_n = 0 ; ch_n < 8 ; ch_n++) {
		memcpy(&reg, &config->filter[ch_n], 1);
		max22190_reg_transceive(dev, MAX22190_FILTER_IN_REG(ch_n), reg,
								MAX22190_WRITE);
	}
}
...
static int max22190_fault_set(const struct device *dev)
{
	...
	memcpy(&reg, &data->fault1_en, 1);
	ret = max22190_reg_transceive(dev, MAX22190_FAULT1_EN_REG, reg, MAX22190_WRITE);
	if (ret)
		goto exit_fault_set;

	memcpy(&reg, &data->fault1, 1);
	ret = max22190_reg_transceive(dev, MAX22190_FAULT1_REG, reg, MAX22190_WRITE);
	if (ret)
		goto exit_fault_set;

	memcpy(&reg, &data->fault2_en, 1);
	ret = max22190_reg_transceive(dev, MAX22190_FAULT2_EN_REG, reg, MAX22190_WRITE);
	if (ret)
		goto exit_fault_set;

	memcpy(&reg, &data->fault2, 1);
	ret = max22190_reg_transceive(dev, MAX22190_FAULT2_REG, reg, MAX22190_WRITE);
	if (ret)
		goto exit_fault_set;
...
static int gpio_max22190_init(const struct device *dev)
{
	...
	for (int i = 0 ; i < 8 ; i++) {
		LOG_DBG("IN%d WBE [%d] FBP[%d] DELAY[%d]\n", i, config->filter[i].max22190_WBE,
				config->filter[i].max22190_FBP, config->filter[i].max22190_DELAY);
	}
	...
	data->fault1_en.max22190_WBGE = 1;
	data->fault1_en.max22190_PORE = 1;
...
#define MAX22190_FILTER_SET_DELAY(delay)	delay == 20000 ? MAX22190_DELAY_20000US :	\
...
#define MAX22190_FILTER_BY_IDX(id, idx) {							\
	.max22190_DELAY = MAX22190_FILTER_SET_DELAY(DT_INST_PROP_BY_IDX(id, filter_delays, idx)),\
	.max22190_FBP = DT_INST_PROP_BY_IDX(id, filter_fbps, idx),				\
	.max22190_WBE = DT_INST_PROP_BY_IDX(id, filter_wbes, idx),				\
}

#define GPIO_MAX22190_DEVICE(id)								\
	static const struct max22190_config max22190_##id##_cfg = {				\
		.spi = SPI_DT_SPEC_INST_GET(id, SPI_OP_MODE_MASTER |				\
						SPI_WORD_SET(8U), 0U),				\
		...
		.filter = {									\
			MAX22190_FILTER_BY_IDX(id, 0), MAX22190_FILTER_BY_IDX(id, 1),		\
			MAX22190_FILTER_BY_IDX(id, 2), MAX22190_FILTER_BY_IDX(id, 3),		\
			MAX22190_FILTER_BY_IDX(id, 4), MAX22190_FILTER_BY_IDX(id, 5),		\
			MAX22190_FILTER_BY_IDX(id, 6), MAX22190_FILTER_BY_IDX(id, 7),		\
		},										\
	};											\
												\
