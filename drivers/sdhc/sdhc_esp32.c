...
static int sdhc_esp32_set_io(const struct device *dev, struct sdhc_io *ios)
{
	...
	LOG_INF("SDHC I/O: bus width %d, clock %dHz, card power %s, voltage %s", ios->bus_width,
...
static void sdhc_esp32_init_host_props(const struct device *dev)
{
	...
	props->host_caps.high_spd_support = (cfg->bus_width_cfg == 4) ? true : false;
	...
	props->host_caps.bus_4_bit_support = (cfg->bus_width_cfg == 4) ? true : false;
...
#define SDHC_ESP32_INIT(n)                                                                         \
	...
	static const struct sdhc_esp32_config sdhc_esp32_##n##_config = {                          \
		...
		.sdio_hw = (const sdmmc_dev_t *)DT_REG_ADDR(DT_NODELABEL(sdio##n)),                \
		.irq_source = DT_INST_IRQN(n),                                                     \
		.bus_width_cfg = DT_INST_PROP(n, bus_width),                                       \
		.pcfg = PINCTRL_DT_DEV_CONFIG_GET(DT_NODELABEL(sdio##n)),                          \
		...
		.power_delay_ms = DT_INST_PROP_OR(n, power_delay_ms, 0),                           \
