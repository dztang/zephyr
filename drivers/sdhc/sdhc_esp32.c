...
#define DT_DRV_COMPAT espressif_esp32_sdhc
...
struct sdhc_esp32_slot_config {

	const int num; /* physical slot number of the peripheral */
...
struct sdhc_esp32_config {
	...
	int slots;
	const struct sdhc_esp32_slot_config *slots_cfg;
...
struct sdhc_esp32_data {
	...
	struct sdhc_esp32_slot_data *slots_data;
...
static int sdhc_esp32_set_io(const struct device *dev, struct sdhc_io *ios)
{
	...
	/* ESP32 supports 2 slots, but only one is supported at the moment.
	 * Clock operations will affect both slots. Individual settings
	 * (bus width, power mode, timing) will be applied to the sole slot
	 * instance allowed, which is index 0 on slots structures
	 */

	LOG_INF("SDHC I/O: slot %d, bus width %d, clock %dHz, card power %s, voltage %s",
		cfg->slots_cfg[slot].num, ios->bus_width, ios->clock,
	...
	if (ios->clock) {
		...
		if (data->bus_clock != (uint32_t)ios->clock) {
			...
			for (int i = 0; i < cfg->slots; i++) {
				ret = sdmmc_host_set_card_clk(sdio_hw, cfg->slots_cfg[i].num,
							      (ios->clock / 1000));

				if (ret == 0) {
					LOG_INF("Bus clock successfully set to %d kHz on slot %d",
						ios->clock / 1000, cfg->slots_cfg[i].num);
				} else {
					LOG_ERR("Error configuring card clock on slot %d",
						cfg->slots_cfg[i].num);
	...
	if (ios->bus_width > 0) {
		...
		if (data->slots_data[slot].bus_width != bus_width) {
			ret = sdmmc_host_set_bus_width(sdio_hw, cfg->slots_cfg[slot].num,
						       bus_width);

			if (ret == 0) {
				LOG_INF("Bus width set successfully to %d bit on slot %d",
					bus_width, cfg->slots_cfg[slot].num);
			} else {
				LOG_ERR("Error configuring bus width on slot %d",
					cfg->slots_cfg[slot].num);
				return err_esp2zep(ret);
			}

			data->slots_data[slot].bus_width = bus_width;
		}
	}

	/* Toggle card power supply */
	if ((data->slots_data[slot].power_mode != ios->power_mode) &&
	    (cfg->slots_cfg[slot].pwr_gpio.port)) {
		if (ios->power_mode == SDHC_POWER_OFF) {
			gpio_pin_set_dt(&cfg->slots_cfg[slot].pwr_gpio, 0);
		} else if (ios->power_mode == SDHC_POWER_ON) {
			gpio_pin_set_dt(&cfg->slots_cfg[slot].pwr_gpio, 1);
		}
		data->slots_data[slot].power_mode = ios->power_mode;
	}

	if (ios->timing > 0) {
		/* Set I/O timing */
		if (data->slots_data[slot].timing != ios->timing) {
			switch (ios->timing) {
			case SDHC_TIMING_LEGACY:
			case SDHC_TIMING_HS:
				sdmmc_ll_enable_ddr_mode(sdio_hw, cfg->slots_cfg[slot].num, false);
				break;
			case SDHC_TIMING_DDR50:
			case SDHC_TIMING_DDR52:
				/* Enable DDR mode */
				sdmmc_ll_enable_ddr_mode(sdio_hw, cfg->slots_cfg[slot].num, true);
			...
			LOG_INF("Bus timing successfully changed to %s for slot %d",
				timingStr[ios->timing], cfg->slots_cfg[slot].num);
			data->slots_data[slot].timing = ios->timing;
		}
	}
			      ...
			      struct sdhc_data *data)
{
	...
	/* ESP32 supports 2 slots, but only one is supported at the moment.
	 * Data operations are only executed for the only slot instance allowed
	 * which is index 0 in the slot structures
	 */
	...
	while (retries > 0) {
		...
		ret_esp = sdmmc_host_do_transaction(dev, cfg->slots_cfg[slot].num, &esp_cmd);
...
static int sdhc_esp32_get_card_present(const struct device *dev)
{
	...
	return sdmmc_ll_is_card_detected(sdio_hw, cfg->slots_cfg[slot].num);
}

/*
 * Initialize SDHC host properties for use in get_host_props api call
 */
static void sdhc_esp32_init_host_props(const struct device *dev)
{
	const struct sdhc_esp32_config *cfg = dev->config;
	struct sdhc_esp32_data *data = dev->data;
	struct sdhc_host_caps caps;
	...
	props->power_delay = cfg->slots_cfg[slot].power_delay_ms;
	...
	props->host_caps.sdma_support = true;
	props->host_caps.high_spd_support =
		(cfg->slots_cfg[slot].bus_width_cfg == 4) ? true : false;
	props->host_caps.adma_2_support = false;
	props->host_caps.max_blk_len = 0;
	props->host_caps.ddr50_support = false;
	props->host_caps.sdr104_support = false;
...
static int sdhc_esp32_init(const struct device *dev)
{
	...
	for (int i = 0; i < cfg->slots; i++) {
		...
		if (cfg->slots_cfg[i].pwr_gpio.port) {
			ret = gpio_pin_configure_dt(&cfg->slots_cfg[i].pwr_gpio,
						    GPIO_OUTPUT_ACTIVE);
		 ...
		 * please use pinctrl for pin configuration.
		 */
		configure_pin_iomux(cfg->slots_cfg[i].clk_pin);
		configure_pin_iomux(cfg->slots_cfg[i].cmd_pin);
		configure_pin_iomux(cfg->slots_cfg[i].d0_pin);
		configure_pin_iomux(cfg->slots_cfg[i].d1_pin);
		configure_pin_iomux(cfg->slots_cfg[i].d2_pin);
		configure_pin_iomux(cfg->slots_cfg[i].d3_pin);

		ret = pinctrl_apply_state(cfg->slots_cfg[i].pcfg, PINCTRL_STATE_DEFAULT);
	...
	sdhc_esp32_init_host_props(dev);
	...
	for (int i = 0; i < cfg->slots; i++) {
		...
		if (ret != 0) {
			LOG_ERR("Error configuring card clock on slot %d", cfg->slots_cfg[i].num);
			return err_esp2zep(ret);
		}

		ret = sdmmc_host_set_bus_width(sdio_hw, cfg->slots_cfg[i].num,
					       SDMMC_SLOT_WIDTH_DEFAULT);

		if (ret != 0) {
			LOG_ERR("Error configuring bus width on slot %d", cfg->slots_cfg[i].num);
			return err_esp2zep(ret);
		}

		/* Data init */
		data->slots_data[i].bus_width = SDMMC_SLOT_WIDTH_DEFAULT;
		data->slots_data[i].power_mode = SDHC_POWER_ON;
		data->slots_data[i].timing = SDHC_TIMING_LEGACY;
	}
...
#define SLOT_CONFIG(id)                                                                    \
	{                                                                                      \
		.num = DT_PROP(id, reg),                                                           \
		.bus_width_cfg = DT_PROP(id, bus_width),                                           \
		.pcfg = PINCTRL_DT_DEV_CONFIG_GET(id),                                             \
		.pwr_gpio = GPIO_DT_SPEC_GET_OR(id, pwr_gpios, {0}),                               \
		.clk_pin = DT_PROP_OR(id, clk_pin, GPIO_NUM_NC),                                   \
		.cmd_pin = DT_PROP_OR(id, cmd_pin, GPIO_NUM_NC),                                   \
		.d0_pin = DT_PROP_OR(id, d0_pin, GPIO_NUM_NC),                                     \
		.d1_pin = DT_PROP_OR(id, d1_pin, GPIO_NUM_NC),                                     \
		.d2_pin = DT_PROP_OR(id, d2_pin, GPIO_NUM_NC),                                     \
		.d3_pin = DT_PROP_OR(id, d3_pin, GPIO_NUM_NC),                                     \
		.power_delay_ms = DT_PROP_OR(id, power_delay_ms, 0),                               \
	},
...
#define SDHC_ESP32_INIT(n)                                                                     \
	...
	K_MSGQ_DEFINE(sdhc##n##_queue, sizeof(struct sdmmc_event), SDMMC_EVENT_QUEUE_LENGTH, 1);   \
                                                                                               \
	DT_FOREACH_CHILD_STATUS_OKAY_SEP(DT_INST_CHILD(n, slots), PINCTRL_DT_DEFINE, ";");         \
                                                                                               \
	static const struct sdhc_esp32_slot_config sdhc_esp32_##n##_slot_config[] = {              \
		DT_FOREACH_CHILD_STATUS_OKAY(DT_INST_CHILD(n, slots), SLOT_CONFIG)};               \
                                                                                                   \
	BUILD_ASSERT(ARRAY_SIZE(sdhc_esp32_##n##_slot_config) == 1,                                \
		     "Only one SD slot is currently supported");                                   \
                                                                                                   \
	static const struct sdhc_esp32_config sdhc_esp32_##n##_config = {                          \
		.slots = ARRAY_SIZE(sdhc_esp32_##n##_slot_config),                                 \
		.slots_cfg = &sdhc_esp32_##n##_slot_config[0],                                     \
		.sdio_hw = (const sdmmc_dev_t *)DT_REG_ADDR(DT_NODELABEL(sdhc)),                   \
		...
		.max_bus_freq = DT_INST_PROP(n, max_bus_freq),                                     \
                                                                                                   ...
                                                                                                   \
	static struct sdhc_esp32_slot_data                                                         \
		sdhc_esp32_##n##_slot_data[ARRAY_SIZE(sdhc_esp32_##n##_slot_config)];              \
                                                                                                   \
	static struct sdhc_esp32_data sdhc_esp32_##n##_data = {                                    \
		.slots_data = &sdhc_esp32_##n##_slot_data[0],                                      \
...
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) == 1,
	     "Only one instance of espressif,esp32-sdhc is currently supported");
