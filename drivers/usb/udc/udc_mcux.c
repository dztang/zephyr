			...
			struct net_buf *const buf)
{
	...
	usb_mcux_control(dev, kUSB_DeviceControlGetEndpointStatus, &ep_status);
			...
			struct udc_ep_config *const cfg)
{
	...
	if (feed_buf) {
		int ret = usb_mcux_ep_feed(dev, cfg, feed_buf);
...
usb_status_t USB_DeviceNotificationTrigger(void *handle, void *msg)
{
	...
	default:
		...
		if (mcux_msg->isSetup) {
			...
			usb_mcux_ep_try_feed(dev, udc_get_ep_cfg(dev, ep));
		}
		break;
	}
			       ...
			       struct net_buf *const buf)
{
	...
	return usb_mcux_ep_try_feed(dev, cfg);
}
...
static int usb_mcux_ep_clear_halt(const struct device *dev,
				  struct udc_ep_config *const cfg)
{
	(void)usb_mcux_control(dev, kUSB_DeviceControlEndpointUnstall, &cfg->addr);
