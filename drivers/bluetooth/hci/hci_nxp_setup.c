...
static const struct device *uart_dev = DEVICE_DT_GET(DT_CHOSEN(zephyr_bt_uart));

#define HCI_UART_CHILD_PREFIX(x) x
#define HCI_UART_CHILD           DT_FOREACH_CHILD(DT_CHOSEN(zephyr_bt_uart), HCI_UART_CHILD_PREFIX)
#if DT_NODE_HAS_PROP(HCI_UART_CHILD, sdio_reset_gpios)
struct gpio_dt_spec sdio_reset = GPIO_DT_SPEC_GET(HCI_UART_CHILD, sdio_reset_gpios);
#endif /* DT_NODE_HAS_PROP(HCI_UART_CHILD, sdio_reset_gpios) */
#if DT_NODE_HAS_PROP(HCI_UART_CHILD, w_disable_gpios)
struct gpio_dt_spec w_disable = GPIO_DT_SPEC_GET(HCI_UART_CHILD, w_disable_gpios);
#endif /* DT_NODE_HAS_PROP(HCI_UART_CHILD, w_disable_gpios) */
...
static unsigned long fw_upload_updateCrc(unsigned long crc_accum, char *data_blk_ptr,
					 int data_blk_size)
{
	unsigned int i, j;
...
{
	while (len > 0) {
		k_sem_take(&fw_upload.rx.sem, K_FOREVER);
...
static int fw_upload_wait_for_hdr_sig(void)
{
	...
	end = k_uptime_get() + CONFIG_BT_NXP_CTLR_WAIT_HDR_SIG_TIMEOUT;
...
static int fw_upload_checkReqCrc(uint8_t *buffer, uint8_t uiReq)
{
	uint8_t crc;
...
static int fw_upload_wait_req(bool secondary_speed)
{
	...
	err = fw_upload_checkReqCrc(buffer, fw_upload.hdr_sig);
	if (err != 0) {
	...
	if (fw_upload.hdr_sig == V3_HEADER_DATA_REQ) {
		...
		chipid = sys_get_le16(&buffer[1]);
		LOG_DBG("Indicate: %hhd, %hd, %hhd, %hhd", fw_upload.hdr_sig, chipid, buffer[3],
			buffer[4]);
...
static int fw_upload_write_hdr_and_data(uint16_t len_to_send, uint8_t *buffer, bool new_speed)
{
	int err;
	uint32_t payload_len;
...
static int fw_upload_change_speed(uint8_t hdr)
{
	...
	crc = fw_upload_updateCrc(0, (char *)cmd5_hdrData, 12);
	crc = sys_cpu_to_be32(crc);
	memcpy(cmd5_hdrData + 12, &crc, CRC32_LEN);
	crc = fw_upload_updateCrc(0, (char *)fw_upload.change_speed_buffer,
	...
	while (true) {
		...
		if (fw_upload.version == VER1) {
			...
			if (len_to_send == V1_START_INDICATION) {
				...
				err = fw_upload_write_hdr_and_data(len_to_send,
...
static int fw_upload_v1_send_data(uint16_t len)
{
	...
	ret_len = fw_upload_write_hdr_and_data(len, fw_upload.send_buffer, false);
...
static int bt_nxp_ctlr_init(void)
{
	...
	speed = DT_PROP(DT_CHOSEN(zephyr_bt_uart), current_speed);
	speed = DT_PROP_OR(HCI_UART_CHILD, hci_operation_speed, speed);
	uart_dev_data.primary_speed = DT_PROP_OR(HCI_UART_CHILD, fw_download_primary_speed, speed);
	uart_dev_data.secondary_speed =
		DT_PROP_OR(HCI_UART_CHILD, fw_download_secondary_speed, speed);

	flowcontrol_of_hci = (bool)DT_PROP_OR(HCI_UART_CHILD, hw_flow_control, false);
	uart_dev_data.primary_flowcontrol =
		(bool)DT_PROP_OR(HCI_UART_CHILD, fw_download_primary_flowcontrol, false);
	uart_dev_data.secondary_flowcontrol =
		(bool)DT_PROP_OR(HCI_UART_CHILD, fw_download_secondary_flowcontrol, false);

#if DT_NODE_HAS_PROP(HCI_UART_CHILD, sdio_reset_gpios) ||                                          \
	DT_NODE_HAS_PROP(HCI_UART_CHILD, w_disable_gpios)
#if DT_NODE_HAS_PROP(HCI_UART_CHILD, sdio_reset_gpios)
	...
	k_sleep(K_MSEC(CONFIG_BT_NXP_CTLR_WAIT_TIME_AFTER_UPLOAD));
