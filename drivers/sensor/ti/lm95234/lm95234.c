				     ...
				     enum sensor_channel chan, int32_t *output)
{
	...
	*output = 0;
	if ((uint32_t)chan >= SENSOR_CHAN_LM95234_REMOTE_TEMP_1) {
		uint16_t temp;
	...
	if ((uint32_t)chan < SENSOR_CHAN_LM95234_REMOTE_TEMP_1 || *output == 0) {
		int offset = (uint32_t)chan < SENSOR_CHAN_LM95234_REMOTE_TEMP_1 ? 0 :
...
int lm95234_init(const struct device *dev)
{
	const struct lm95234_config *cfg = dev->config;
	int ret = 0;
