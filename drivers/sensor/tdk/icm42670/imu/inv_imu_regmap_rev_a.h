...
/*
 * mclk_rdy * 0: Indicates internal clock is currently not running
...
/*
 * spi_ap_4wire * 0: AP interface uses 3-wire SPI mode
...
/*
 * spi_mode * SPI mode selection
 * * 0: Mode 0 and Mode 3
 * 1: Mode 1 and Mode 2
 * * If device is operating in non-SPI mode, user is not allowed to change the power-on default
 * setting of this register. Change of this register setting will not take effect till AP_CS = 1.
 */
...
/*
 * i3c_ddr_slew_rate * Controls slew rate for output pin 14 when device is in I3CSM DDR protocol.
 * While in I3CSM operation, the device automatically switches to use I3C_DDR_SLEW_RATE after
 * receiving ENTHDR0 ccc command from the host. The device automatically switches back to
 * I3C_SDR_SLEW_RATE after the host issues HDR_EXIT pattern. * * 000: MIN: 20 ns; TYP: 40 ns; MAX:
 * 60 ns 001: MIN: 12 ns; TYP: 24 ns; MAX: 36 ns 010: MIN: 6 ns; TYP: 12 ns; MAX: 19 ns 011: MIN: 4
 * ns; TYP: 8 ns; MAX: 14 ns 100: MIN: 2 ns; TYP: 4 ns; MAX: 8 ns 101: MAX: 2 ns 110: Reserved 111:
 * Reserved
...
/*
 * i3c_sdr_slew_rate * Controls slew rate for output pin 14 in I3CSM SDR protocol.
 * After device reset, I2C_SLEW_RATE is used by default. If I3CSM feature is enabled, the device
 * automatically switches to use I3C_SDR_SLEW_RATE after receiving 0x7E+W message (an I3CSM
 * broadcast message).
...
/*
 * i2c_slew_rate * Controls slew rate for output pin 14 in I2C mode.
 * After device reset, the I2C_SLEW_RATE is used by default. If the 1st write operation from host is
 * an SPI transaction, the device automatically switches to SPI_SLEW_RATE. If I3CSM feature is
 * enabled, the device automatically switches to I3C_SDR_SLEW_RATE after receiving 0x7E+W message
 * (an I3C broadcast message).
 * * 000: MIN: 20 ns; TYP: 40 ns; MAX: 60 ns
 * 001: MIN: 12 ns; TYP: 24 ns; MAX: 36 ns
 * 010: MIN: 6 ns; TYP: 12 ns; MAX: 19 ns
 * 011: MIN: 4 ns; TYP: 8 ns; MAX: 14 ns
...
/*
 * all_slew_rate * Configure drive strength for all output pins in all modes (SPI3, SPI4, I2C,
...
/*
 * spi_slew_rate * Controls slew rate for output pin 14 in SPI 3-wire mode. In SPI 4-wire mode this
 * register controls the slew rate of pin 1 as it is used as an output in SPI 4-wire mode only.
 * After chip reset, the I2C_SLEW_RATE is used by default for pin 14 pin. If the 1st write operation
 * from the host is an SPI3/4 transaction, the device automatically switches to SPI_SLEW_RATE.
 * * 000: MIN: 20 ns; TYP: 40 ns; MAX: 60 ns
 * 001: MIN: 12 ns; TYP: 24 ns; MAX: 36 ns
 * 010: MIN: 6 ns; TYP: 12 ns; MAX: 19 ns
 * 011: MIN: 4 ns; TYP: 8 ns; MAX: 14 ns
 ...
 * 101: MAX: 2 ns
 * 110: Reserved
 * 111: Reserved * * This register field should not be programmed in I3C/DDR mode
 */
...
/*
 * int2_mode * Interrupt mode and drive circuit shall be configurable by register. * Interrupt Mode
 * 1: Latched Mode
 * 0: Pulsed Mode
 */
...
/*
 * int2_drive_circuit * Interrupt mode and drive circuit shall be configurable by register. * Drive
 * Circuit 1: Push-Pull 0: Open drain
 */
...
/*
 * int2_polarity * Interrupt mode and drive circuit shall be configurable by register. * Interrupt
...
/*
 * int1_mode * Interrupt mode and drive circuit shall be configurable by register. * Interrupt Mode
 * 1: Latched Mode
 * 0: Pulsed Mode
 */
...
/*
 * int1_drive_circuit * Interrupt mode and drive circuit shall be configurable by register. * Drive
 * Circuit 1: Push-Pull 0: Open drain
 */
...
/*
 * int1_polarity * Interrupt mode and drive circuit shall be configurable by register. * Interrupt
 * Polarity 1: Active High 0: Active Low
 */
...
/*
 * accel_data_x * Accel X axis data
 */
...
/*
 * accel_data_y * Accel Y axis data
 */
...
/*
 * accel_data_z * Accel Z axis data
 */
...
/*
 * tmst_fsync_data * Stores the time delta from the  rising edge of FSYNC to the latest ODR until
...
/*
 * ff_dur * Free Fall duration. The duration is given in number of samples and it can be converted
 * to freefall distance by applying the following formula: ff_distance =
 * 0.5*9.81*(ff_duration*dmp_odr_s)^2) Note: dmp_odr_s in the duration of DMP_ODR expressed in
 * seconds.
 */
...
/*
 * accel_lp_clk_sel * 0: Accelerometer LP mode uses Wake Up oscillator clock. This is the lowest
 * power consumption mode and it is the recommended setting. * 1: Accelerometer LP mode uses RC
 * oscillator clock.
...
/*
 * idle * If this bit is set to 1, the  RC oscillator is powered on even if Accel and Gyro are
 * powered off. * Nominally this bit is set to 0, so when Accel and Gyro are powered off, * the chip
 * will go to OFF state , since the RC oscillator will also be powered off. * * This field can be
 * changed on-the-fly even if a sensor is already on
 */
#define PWR_MGMT0_IDLE_POS  0x04
#define PWR_MGMT0_IDLE_MASK (0x01 << PWR_MGMT0_IDLE_POS)

/*
 * gyro_mode * 00: Turns gyroscope off * 01: Places gyroscope in Standby Mode
 * 10: Reserved
 * 11: Places gyroscope in Low Noise (LN) Mode
 * * Gyroscope needs to be kept ON for a minimum of 45ms. When transitioning from OFF to any of the
 * other modes, do not issue any register writes for 200 µs.
...
/*
 * accel_mode * 00: Turns accelerometer off * 01: Turns accelerometer off
 ...
 * * Before entering LP mode and during LP Mode the following combinations of ODR and averaging are
 * not permitted: 1) ODR=1600 Hz or ODR=800 Hz: any averaging. 2) ODR=400 Hz: averaging=16x, 32x or
 * 64x. 3) ODR=200 Hz: averaging=64x.
 * * When transitioning from OFF to any of the other modes, do not issue any register writes for 200
 * µs.
 * * This field can be changed on-the-fly even if accel sensor is on
 */
...
/*
 * temp_filt_bw * Sets the bandwidth of the temperature signal DLPF
 * * 000: DLPF bypassed
 * 001: DLPF BW = 180 Hz
 * 010: DLPF BW = 72 Hz
 * 011: DLPF BW = 34 Hz
...
/*
 * gyro_ui_filt_bw * Selects GYRO UI low pass filter bandwidth
...
/*
 * accel_ui_avg * Selects averaging filter setting to create accelerometer output in accelerometer
 * low power mode (LPM)
 * * 000: 2x average
 * 001: 4x average
 * 010: 8x average
...
/*
 * accel_ui_filt_bw * Selects ACCEL UI low pass filter bandwidth
 * * 000: Low pass filter bypassed
 * 001: 180 Hz
 * 010: 121 Hz
 * 011: 73 Hz
...
/*
 * dmp_power_save_en * When this bit is set to 1, power saving is enabled for DMP algorithms
 */
...
/*
 * dmp_init_en * When this bit is set to 1, DMP runs DMP SW initialization procedure. Bit is reset
 * by hardware when the procedure is finished. All other APEX features are ignored as long as
 * DMP_INIT_EN is set.
 * * This field can be changed on-the-fly even if accel sensor is on.
 */
...
/*
 * dmp_mem_reset_en * When this bit is set to 1, it clears DMP SRAM for APEX operation or Self-test
 * operation.
 */
...
/*
 * smd_enable * 0: Significant Motion Detection not enabled
...
/*
 * ff_enable * 0: Freefall Detection not enabled
...
/*
 * tilt_enable * 0: Tilt Detection not enabled
...
/*
 * ped_enable * 0: Pedometer not enabled
 * 1: Pedometer enabled
 * * This field can be changed on-the-fly even if accel sensor is on
 */
 ...
 * 10: 50 Hz
 * 11: 100 Hz
 * * The ACCEL_ODR field must be configured to an ODR equal or greater to the DMP_ODR field, for
...
/*
 * wom_int_dur * Selects Wake on Motion interrupt assertion from among the following options
 ...
 * * This field can be changed on-the-fly even if accel sensor is on, but it cannot be changed if
 * WOM_EN is already enabled
 */
 ...
 * * This field can be changed on-the-fly even if accel sensor is already on, but it cannot be
 * changed if wom_en is already enabled.
 */
#define WOM_CONFIG_WOM_MODE_POS  0x01
#define WOM_CONFIG_WOM_MODE_MASK (0x01 << WOM_CONFIG_WOM_MODE_POS)

/*
 * wom_en * 1: enable wake-on-motion detection. * 0: disable wake-on-motion detection. * * This
 * field can be changed on-the-fly even if accel sensor is already on.
 */
...
/*
 * fifo_bypass * FIFO bypass control
...
/*
 * fifo_wm * FIFO watermark. Generate interrupt when the FIFO reaches or exceeds FIFO_WM size in
 * bytes or records according to FIFO_COUNT_FORMAT setting. FIFO_WM_EN must be zero before writing
 * this register. Interrupt only fires once. This register should be set to non-zero value, before
 * choosing this interrupt source. * * This field should be changed when FIFO is empty to avoid
 * spurious interrupts.
 */
...
/*
 * st_int1_en * 0: Self-Test Done interrupt not routed to INT1
...
/*
 * fsync_int1_en * 0: FSYNC interrupt not routed to INT1
...
/*
 * pll_rdy_int1_en * 0: PLL ready interrupt not routed to INT1
...
/*
 * reset_done_int1_en * 0: Reset done interrupt not routed to INT1
...
/*
 * drdy_int1_en * 0: Data Ready interrupt not routed to INT1
...
/*
 * fifo_ths_int1_en * 0: FIFO threshold interrupt not routed to INT1
...
/*
 * fifo_full_int1_en * 0: FIFO full interrupt not routed to INT1
...
/*
 * agc_rdy_int1_en * 0: UI AGC ready interrupt not routed to INT1
...
/*
 * i3c_protocol_error_int1_en * 0: I3CSM protocol error interrupt not routed to INT1
 * 1: I3CSM protocol error interrupt routed to INT1
 */
#define INT_SOURCE1_I3C_PROTOCOL_ERROR_INT1_EN_POS 0x06
#define INT_SOURCE1_I3C_PROTOCOL_ERROR_INT1_EN_MASK                                                \
	(0x01 << INT_SOURCE1_I3C_PROTOCOL_ERROR_INT1_EN_POS)

/*
 * smd_int1_en * 0: SMD interrupt not routed to INT1
...
/*
 * wom_z_int1_en * 0: Z-axis WOM interrupt not routed to INT1
...
/*
 * wom_y_int1_en * 0: Y-axis WOM interrupt not routed to INT1
...
/*
 * wom_x_int1_en * 0: X-axis WOM interrupt not routed to INT1
...
/*
 * st_int2_en * 0: Self-Test Done interrupt not routed to INT2
...
/*
 * fsync_int2_en * 0: FSYNC interrupt not routed to INT2
...
/*
 * pll_rdy_int2_en * 0: PLL ready interrupt not routed to INT2
...
/*
 * drdy_int2_en * 0: Data Ready interrupt not routed to INT2
...
/*
 * fifo_ths_int2_en * 0: FIFO threshold interrupt not routed to INT2
...
/*
 * fifo_full_int2_en * 0: FIFO full interrupt not routed to INT2
...
/*
 * agc_rdy_int2_en * 0: AGC ready interrupt not routed to INT2
...
/*
 * i3c_protocol_error_int2_en * 0: I3CSM protocol error interrupt not routed to INT2
 * 1: I3CSM protocol error interrupt routed to INT2
 */
#define INT_SOURCE4_I3C_PROTOCOL_ERROR_INT2_EN_POS 0x06
#define INT_SOURCE4_I3C_PROTOCOL_ERROR_INT2_EN_MASK                                                \
	(0x01 << INT_SOURCE4_I3C_PROTOCOL_ERROR_INT2_EN_POS)

/*
 * smd_int2_en * 0: SMD interrupt not routed to INT2
...
/*
 * wom_z_int2_en * 0: Z-axis WOM interrupt not routed to INT2
...
/*
 * wom_y_int2_en * 0: Y-axis WOM interrupt not routed to INT2
...
/*
 * wom_x_int2_en * 0: X-axis WOM interrupt not routed to INT2
...
/*
 * fifo_lost_pkt_cnt * Stores the  number of packets lost in the FIFO
 */
...
/*
 * step_cnt * This status register indicates number of step taken.
 */
...
/*
 * step_cadence * Pedometer step cadence.Walk/run cadency in number of samples. Format is u6.2. *
...
/*
 * dmp_idle * 0: Indicates DMP is running
...
/*
 * fifo_count_format * 0: FIFO count is reported in bytes
 * 1: FIFO count is reported in records (1 record = 16 bytes for header + gyro + accel + temp sensor
 * data + time stamp, or 8 bytes for header + gyro/accel + temp sensor data)
 */
...
/*
 * fifo_count_endian * This bit applies to both fifo_count and lost_pkt_count
...
/*
 * sensor_data_endian * This bit applies to sensor data to AP, and fifo data.
 * 0 : Little Endian (The LSByte data is read first, followed by MSByte data).
 * 1 : Big Endian (The MSByte data is read first, followed by LSByte data).
 */
...
/*
 * i3c_sdr_en * 0: I3CSM SDR mode not enabled
...
/*
 * i3c_ddr_en * 0: I3CSM DDR mode not enabled
...
/*
 * clksel * 00 Alway select internal RC oscillator * 01 Select PLL when available, else select RC
 * oscillator (default) 10 (Reserved) * 11 Disable all clocks
 */
...
/*
 * data_rdy_int * This bit automatically sets to 1 when a Data Ready interrupt is generated. The bit
...
/*
 * st_int * This bit automatically sets to 1 when a Self Test done interrupt is generated. The bit
 * clears to 0 after the register has been read.
 */
...
/*
 * fsync_int * This bit automatically sets to 1 when an FSYNC interrupt is generated. The bit clears
...
/*
 * pll_rdy_int * This bit automatically sets to 1 when a PLL Ready interrupt is generated. The bit
 * clears to 0 after the register has been read.
 */
...
/*
 * reset_done_int * This bit automatically sets to 1 when software reset is complete. The bit clears
 * to 0 after the register has been read.
 */
...
/*
 * fifo_ths_int * This bit automatically sets to 1 when the FIFO buffer reaches the threshold value.
...
/*
 * fifo_full_int * This bit automatically sets to 1 when the FIFO buffer is full. The bit clears to
...
/*
 * agc_rdy_int * This bit automatically sets to 1 when an AGC Ready interrupt is generated. The bit
 * clears to 0 after the register has been read.
 */
...
/*
 * step_det_int * Step Detection Interrupt, clears on read
 */
#define INT_STATUS3_STEP_DET_INT_POS  0x05
#define INT_STATUS3_STEP_DET_INT_MASK (0x01 << INT_STATUS3_STEP_DET_INT_POS)

/*
 * step_cnt_ovf_int * Step Count Overflow Interrupt, clears on read
 */
...
/*
 * fifo_count * Number of bytes in FIFO when fifo_count_format=0.
...
/*
 * whoami * Register to indicate to user which device is being accessed
 */
...
/*
 * blk_sel_w * For write operation, select a 256-byte MCLK space, or 128-byte SCLK space.
...
/*
 * maddr_w * For MREG write operation, the lower 8-bit address for accessing MCLK domain registers.
 */
...
/*
 * m_w * For MREG write operation, the write port for accessing MCLK domain registers.
 */
...
/*
 * blk_sel_r * For read operation, select a 256-byte MCLK space, or 128-byte SCLK space.
 * Automatically reset when OTP copy operation is triggered.
 */
...
/*
 * maddr_r * For MREG read operation, the lower 8-bit address for accessing MCLK domain registers.
 */
...
/*
 * m_r * For MREG read operation, the read port for accessing MCLK domain registers.
 */
...
/*
 * tmst_res * Time Stamp resolution; When set to 0 (default), time stamp resolution is 1 us. When
 * set to 1, resolution is 16us
 */
#define TMST_CONFIG1_TMST_RES_POS  0x03
#define TMST_CONFIG1_TMST_RES_MASK (0x01 << TMST_CONFIG1_TMST_RES_POS)

/*
 * tmst_delta_en * Time Stamp delta Enable : When set to 1, the Time stamp field contains the
...
/*
 * tmst_fsync_en * Time Stamp register Fsync Enable . When set to 1, the contents of the Timestamp
 * feature of  FSYNC  is enabled. The  user also needs to select  fifo_tmst_fsync_en in order to
...
/*
 * tmst_en * Time Stamp register Enable
 */
...
/*
 * fifo_wm_gt_th * 1: trigger FIFO-Watermark interrupt on every ODR(DMA Write)  if FIFO_COUNT:
 * =FIFO_WM * * 0: Trigger FIFO-Watermark interrupt when FIFO_COUNT == FIFO_WM
 */
#define FIFO_CONFIG5_FIFO_WM_GT_TH_POS  0x05
#define FIFO_CONFIG5_FIFO_WM_GT_TH_MASK (0x01 << FIFO_CONFIG5_FIFO_WM_GT_TH_POS)

/*
 * fifo_resume_partial_rd * 0: FIFO is read in packets. If a partial packet is read, then the
 * subsequent read will start from the beginning of the un-read packet. 1: FIFO can be read
 * partially. When read is resumed, FIFO bytes will  continue  from last read point. The  SW driver
 * is responsible for cascading previous read and present read and maintain frame boundaries.
 */
...
/*
 * fifo_hires_en * Allows 20 bit resolution in the  FIFO packet readout
 */
#define FIFO_CONFIG5_FIFO_HIRES_EN_POS  0x03
#define FIFO_CONFIG5_FIFO_HIRES_EN_MASK (0x01 << FIFO_CONFIG5_FIFO_HIRES_EN_POS)

/*
 * fifo_tmst_fsync_en * Allows the TMST in the FIFO to be replaced by the FSYNC timestamp
 */
...
/*
 * fifo_empty_indicator_dis * 0: xFF is sent out as FIFO data when FIFO is empty.
 * 1: The last FIFO data is sent out when FIFO is empty.
 */
#define FIFO_CONFIG6_FIFO_EMPTY_INDICATOR_DIS_POS 0x04
#define FIFO_CONFIG6_FIFO_EMPTY_INDICATOR_DIS_MASK                                                 \
	(0x01 << FIFO_CONFIG6_FIFO_EMPTY_INDICATOR_DIS_POS)

/*
 * rcosc_req_on_fifo_ths_dis * 0: When the FIFO is operating in ALP+WUOSC mode and the watermark
 * (WM) interrupt is enabled, the FIFO wakes up the system oscillator (RCOSC) as soon as the
 * watermark level is reached. The system oscillator remains enabled until a Host FIFO read
 * operation happens. This will temporarily cause a small increase in the power consumption due to
 * the enabling of the system oscillator. 1: The system oscillator is not automatically woken-up by
...
/*
 * fsync_ui_sel * this register was called (ext_sync_sel)
...
/*
 * fsync_ui_flag_clear_sel * 0 means the FSYNC flag is cleared when UI sensor reg is updated
 * 1 means the FSYNC flag is cleared when UI interface  reads the sensor register LSB of FSYNC
 * tagged axis
 */
...
/*
 * fsync_polarity * 0: Start from Rising edge of FSYNC pulse to measure FSYNC interval
...
/*
 * ui_drdy_int_clear * Data Ready Interrupt Clear Option (latched mode)
...
/*
 * fifo_full_int_clear * FIFO Full Interrupt Clear Option (latched mode)
 * 00: Clear on Status Bit Read
 * 01: Clear on Status Bit Read
 * 10: Clear on FIFO data 1Byte Read
 * 11: Clear on Status Bit Read OR on FIFO data 1 byte read
...
/*
 * int_tpulse_duration * 0 - (Default) Interrupt pulse duration is 100us
...
/*
 * int_async_reset * 0: The interrupt pulse is reset as soon as the interrupt status register is
 * read if the pulse is still active. 1: The interrupt pulse remains high for the intended duration
 * independent of when the interrupt status register is read. This is the default and recommended
 * setting. In this case, when in ALP with the WUOSC clock, the clearing of the interrupt status
 * register requires up to one ODR period after reading.
 */
...
/*
 * apex_disable * 1: Disable APEX features to extend FIFO size to 2.25 Kbytes
 */
...
/*
 * accel_st_reg * User must set this bit to 1 when enabling accelerometer self-test and clear it to
 * 0 when self-test procedure has completed.
 */
#define ST_CONFIG_ACCEL_ST_REG_POS  0x07
#define ST_CONFIG_ACCEL_ST_REG_MASK (0x01 << ST_CONFIG_ACCEL_ST_REG_POS)

/*
 * st_number_sample * This bit selects the number of sensor samples that should be used to process
 * self-test 0: 16 samples 1: 200 samples
 */
#define ST_CONFIG_ST_NUMBER_SAMPLE_POS  0x06
#define ST_CONFIG_ST_NUMBER_SAMPLE_MASK (0x01 << ST_CONFIG_ST_NUMBER_SAMPLE_POS)

/*
 * accel_st_lim * These bits control the tolerated ratio between self-test processed values and
 * reference (fused) ones for accelerometer. * 0 : 5% 1: 10% 2: 15% 3: 20% 4: 25% 5: 30% 6: 40% 7:
 * 50%
 */
...
/*
 * gyro_st_lim * These bits control the tolerated ratio between self-test processed values and
 * reference (fused) ones for gyro. * 0 : 5% 1: 10% 2: 15% 3: 20% 4: 25% 5: 30% 6: 40% 7: 50%
 */
...
/*
 * gyro_st_en * 1: enable gyro self test operation. Host needs to program this bit to 0 to move chip
 * out of self test mode. If host programs this bit to 0 while st_busy = 1 and st_done =0, the
 * current running self-test operation is terminated by host.
 */
#define SELFTEST_GYRO_ST_EN_POS  0x07
#define SELFTEST_GYRO_ST_EN_MASK (0x01 << SELFTEST_GYRO_ST_EN_POS)

/*
 * accel_st_en * 1: enable accel self test operation. Host needs to program this bit to 0 to move
 * chip out of self test mode. If host programs this bit to 0 while st_busy = 1 and st_done =0, the
 * current running self-test operation is terminated by host.
 */
...
/*
 * i3c_ibi_en * I3C Enable IBI function.
 */
...
/*
 * asynctime0_dis * 1: Disable asynchronous timing control mode 0 operation.
 */
...
/*
 * i3c_ddr_wr_mode * This bit controls how I3C slave treats the 1st 2-byte data from *     host in a
 * DDR write operation. * *     0: (a) The 1st-byte in DDR-WR configures the starting register
 *            address where the write operation should occur. *        (b) The 2nd-byte in DDR-WR is
 * ignored and dropped. *        (c) The 3rd-byte in DDR-WR will be written into the register * with
 * address specified by the 1st-byte. Or, the next DDR-RD will be starting from the address *
 * specified by the 1st-byte of previous DDR-WR.
 * *     1:  (a) The 1st-byte in DDR-WR configures the starting register *             address where
 * the write operation should occur. *         (b) The 2nd-byte in DDR-WR will be written into the
 * register *             with address specified by the 1st-byte.
 */
...
/*
 * ff_int1_en * 0: Freefall interrupt not routed to INT1
...
/*
 * lowg_int1_en * 0: Low-g interrupt not routed to INT1
...
/*
 * step_det_int1_en * 0: Step detect interrupt not routed to INT1
...
/*
 * step_cnt_ofl_int1_en * 0: Step count overflow interrupt not routed to INT1
...
/*
 * tilt_det_int1_en * 0: Tilt detect interrupt not routed to INT1
...
/*
 * ff_int2_en * 0: Freefall interrupt not routed to INT2
...
/*
 * lowg_int2_en * 0: Low-g interrupt not routed to INT2
...
/*
 * step_cnt_ofl_int2_en * 0: Step count overflow interrupt not routed to INT2
...
/*
 * tilt_det_int2_en * 0: Tilt detect interrupt not routed to INT2
...
/*
 * fsync_ibi_en * 0: FSYNC interrupt not routed to IBI
...
/*
 * pll_rdy_ibi_en * 0: PLL ready interrupt not routed to IBI
...
/*
 * ui_drdy_ibi_en * 0: UI data ready interrupt not routed to IBI
...
/*
 * fifo_ths_ibi_en * 0: FIFO threshold interrupt not routed to IBI
...
/*
 * fifo_full_ibi_en * 0: FIFO full interrupt not routed to IBI
...
/*
 * agc_rdy_ibi_en * 0: AGC ready interrupt not routed to IBI
...
/*
 * i3c_protocol_error_ibi_en * 0: I3CSM protocol error interrupt not routed to IBI
 * 1: I3CSM protocol error interrupt routed to IBI
 */
#define INT_SOURCE9_I3C_PROTOCOL_ERROR_IBI_EN_POS 0x07
#define INT_SOURCE9_I3C_PROTOCOL_ERROR_IBI_EN_MASK                                                 \
	(0x01 << INT_SOURCE9_I3C_PROTOCOL_ERROR_IBI_EN_POS)

/*
 * ff_ibi_en * 0: Freefall interrupt not routed to IBI
...
/*
 * lowg_ibi_en * 0: Low-g interrupt not routed to IBI
...
/*
 * smd_ibi_en * 0: SMD interrupt not routed to IBI
...
/*
 * wom_z_ibi_en * 0: Z-axis WOM interrupt not routed to IBI
...
/*
 * wom_y_ibi_en * 0: Y-axis WOM interrupt not routed to IBI
...
/*
 * wom_x_ibi_en * 0: X-axis WOM interrupt not routed to IBI
...
/*
 * st_done_ibi_en * 0: Self-test done interrupt not routed to IBI
...
/*
 * step_det_ibi_en * 0: Step detect interrupt not routed to IBI
...
/*
 * step_cnt_ofl_ibi_en * 0: Step count overflow interrupt not routed to IBI
...
/*
 * tilt_det_ibi_en * 0: Tilt detect interrupt not routed to IBI
...
/*
 * low_energy_amp_th_sel * Threshold to select a valid step. Used to increase step detection for
 * slow walk use case. * * 0000: 30 mg 0001: 35 mg 0010: 40 mg 0011: 45 mg 0100: 50 mg 0101: 55 mg
 * 0110: 60 mg
 * 0111: 65 mg
 * 1000: 70 mg
 * 1001: 75 mg
...
/*
 * dmp_power_save_time_sel * Duration of the period while the DMP stays awake after receiving a WOM
 * event.
 * * 0000: 0 seconds
 * 0001: 4 seconds
...
/*
 * ped_amp_th_sel * Threshold of step detection sensitivity.
...
/*
 * ped_step_cnt_th_sel * Minimum number of steps that must be detected before step count is
 * incremented.
...
/*
 * ped_step_det_th_sel * Minimum number of steps that must be detected before step event is
 * signaled. * * Low values reduce latency but increase false positives. High values increase step
 * event validity but increase latency.
 * * 000: 0 steps
 * 001: 1 step
 * 010: 2 steps (default)
 * 011: 3 steps
...
/*
 * ped_sb_timer_th_sel * Duration before algorithm considers that user has stopped taking steps.
 * * 000: 50 samples
 * 001: 75 sample
 * 010: 100 samples
 * 011: 125 samples
...
/*
 * ped_hi_en_th_sel * Threshold to classify acceleration signal as motion not due to steps.
...
/*
 * tilt_wait_time_sel * Minimum duration for which the device should be tilted before signaling
 * event.
 * * 00: 0s
 * 01: 2s
 * 10: 4s (default)
...
/*
 * lowg_peak_th_hyst_sel * Hysteresis value added to the low-g threshold after exceeding it.
 * * 000: 31 mg (default)
 * 001: 63 mg
 * 010: 94 mg
 * 011: 125 mg
...
/*
 * highg_peak_th_hyst_sel * Hysteresis value subtracted from the high-g threshold after exceeding
 * it.
 * * 000: 31 mg (default)
 * 001: 63 mg
 * 010: 94 mg
...
/*
 * ff_debounce_duration_sel * Period after a freefall is signaled during which a new freefall will
 * not be detected. Prevents false detection due to bounces.
 * * 0000: 0 ms
 * 0001: 1250 ms
 * 0010: 1375 ms
 * 0011: 1500 ms
...
#define APEX_CONFIG9_FF_DEBOUNCE_DURATION_SEL_MASK                                                 \
	(0x0f << APEX_CONFIG9_FF_DEBOUNCE_DURATION_SEL_POS)

/*
 * smd_sensitivity_sel * Parameter to tune SMD algorithm robustness to rejection, ranging from 0 to
 * 4 (values higher than 4 are reserved).
 * * Low values increase detection rate but increase false positives. * High values reduce false
 * positives but reduce detection rate (especially for transport use cases).
 * * Default value is 0.
 */
...
/*
 * sensitivity_mode * Pedometer sensitivity mode * 0: Normal (default)
 * 1: Slow walk
 * * Slow walk mode improves slow walk detection (<1Hz) but the number of false positives may
 * increase.
 */
...
/*
 * lowg_peak_th_sel * Threshold for accel values below which low-g state is detected.
 * * 00000: 31 mg (default)
 * 00001: 63 mg
 * 00010: 94 mg
 * 00011: 125 mg
...
/*
 * lowg_time_th_sel * Number of samples required to enter low-g state.
...
/*
 * highg_peak_th_sel * Threshold for accel values above which high-g state is detected.
...
/*
 * highg_time_th_sel * Number of samples required to enter high-g state.
 * * 000: 1 sample (default)
 * 001: 2 samples
 * 010: 3 samples
 * 011: 4 samples
...
/*
 * wom_x_th * Threshold value for the Wake on Motion Interrupt for X-axis accelerometer
 * WoM thresholds are expressed in fixed “mg” independent of the selected Range [0g : 1g];
 * Resolution 1g/256=~3.9mg
 */
...
/*
 * wom_y_th * Threshold value for the Wake on Motion Interrupt for Y-axis accelerometer
 * WoM thresholds are expressed in fixed “mg” independent of the selected Range [0g : 1g];
 * Resolution 1g/256=~3.9mg
 */
...
/*
 * wom_z_th * Threshold value for the Wake on Motion Interrupt for Z-axis accelerometer
 * WoM thresholds are expressed in fixed “mg” independent of the selected Range [0g : 1g];
 * Resolution 1g/256=~3.9mg
 */
...
/*
 * gyro_x_offuser * Gyro offset programmed by user. Max value is +/-64 dps, resolution is 1/32 dps
 */
...
/*
 * gyro_y_offuser * Gyro offset programmed by user. Max value is +/-64 dps, resolution is 1/32 dps
 */
...
/*
 * gyro_z_offuser * Gyro offset programmed by user. Max value is +/-64 dps, resolution is 1/32 dps
 */
...
/*
 * accel_x_offuser * Accel offset programmed by user. Max value is +/-1 gee, resolution is 0.5 mgee
 */
...
/*
 * accel_y_offuser * Accel offset programmed by user. Max value is +/-1 gee, resolution is 0.5 mgee
 */
...
/*
 * accel_z_offuser * Accel offset programmed by user. Max value is +/-1 gee, resolution is 0.5 mgee
 */
...
/*
 * fdr_sel * [7:4] Reserved
 * [3:0] FIFO packet rate decimation factor.  Sets the number of discarded FIFO packets.  Valid
 * range is 0 to 127.  User must disable sensors when initializing FDR_SEL value or making changes
 * to it.
...
/*
 * ff_max_duration_sel * Maximum freefall length. Longer freefalls are ignored.
 * * 0000: 102 cm (default)
 * 0001: 120 cm
 * 0010: 139 cm
 * 0011: 159 cm
...
/*
 * ff_min_duration_sel * Minimum freefall length. Shorter freefalls are ignored.
 * * 0000: 10 cm (default)
 * 0001: 12 cm
 * 0010: 13 cm
 * 0011: 16 cm
...
/*
 * xa_st_data * Accel X-axis self test data converted to 8 bit code.
 */
...
/*
 * ya_st_data * Accel Y-axis self test data converted to 8 bit code.
 */
...
/*
 * za_st_data * Accel Z-axis self test data converted to 8 bit code.
 */
...
/*
 * xg_st_data * Gyro X-axis self test data converted to 8 bit code.
 */
...
/*
 * yg_st_data * Gyro Y-axis self test data converted to 8 bit code.
 */
...
/*
 * zg_st_data * Gyro Z-axis self test data converted to 8 bit code.
 */
...
/*
 * otp_reload * 1: to trigger OTP copy operation. This bit is cleared to 0 after OTP copy is done.
...
/*
 * otp_pwr_down * 0: Power up OTP to copy from OTP to SRAM
 * 1: Power down OTP
