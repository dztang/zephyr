...
typedef struct inv_imu_device {
	/** Transport layer.	 *  @warning Must be the first one of inv_imu_device_t
	 */
 ...
 *         * Enables fsync.
 *         * Enables timestamp to registers. Once fsync is enabled fsync counter is pushed to * FIFO
 * instead of timestamp. So timestamp is made available in registers. Note that *           this
 ...
 * instead of fsync counter. So in order to decrease power consumption, timestamp is no * more
...
/** @brief Read all data registers. *         Then it calls sensor_event_cb function passed at init
 * for each packet.
 *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error.
 */
...
/** @brief Read FIFO frame count. *  @param[in] s             Pointer to device.
...
/** @brief Decode FIFO frame. *  @param[in] s       Pointer to device.
 ...
 * sensor_event_cb function passed *         at init for each packet.
 *  @param[in] s  Pointer to device.
...
/** @brief Enable Wake On Motion.
 *         WoM requests to have the accelerometer enabled to work. *         As a consequence FIFO
 * water-mark interrupt is disabled to only trigger WoM interrupts. To have good performance, it's
 * recommended to set accel ODR to 20 ms and in Low Power Mode. *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error.
 */
...
/** @brief Converts two bytes in one unsigned half-word depending on endianness *  @param[in]
 * endianness  IMU's endianness.
...
/** @brief Converts two bytes in one signed half-word depending on endianness *  @param[in]
 * endianness  IMU's endianness.
 *  @param[in]  in         Pointer to input.
 *  @param[out] out        Pointer to output.
 */
