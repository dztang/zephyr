				      ...
				      const inv_imu_apex_parameters_t *apex_inputs)
{
	...
	/* APEX_CONFIG2_MREG1 */
	apexConfig[0] =
		(uint8_t)apex_inputs->power_save_time | (uint8_t)apex_inputs->low_energy_amp_th;
	...
	/* APEX_CONFIG11_MREG1 */
	apexConfig[2] =
		(uint8_t)apex_inputs->highg_peak_th | (uint8_t)apex_inputs->highg_samples_th;
...
int inv_imu_apex_get_parameters(inv_imu_device_t *s, inv_imu_apex_parameters_t *apex_params)
{
	...
	apex_params->power_save_time =
		(APEX_CONFIG2_DMP_POWER_SAVE_TIME_t)(data[0] &
						     APEX_CONFIG2_DMP_POWER_SAVE_TIME_SEL_MASK);
	apex_params->low_energy_amp_th =
		(APEX_CONFIG2_LOW_ENERGY_AMP_TH_t)(data[0] &
						   APEX_CONFIG2_LOW_ENERGY_AMP_TH_SEL_MASK);
	...
	apex_params->lowg_peak_hyst =
		(APEX_CONFIG5_LOWG_PEAK_TH_HYST_t)(data[3] &
						   APEX_CONFIG5_LOWG_PEAK_TH_HYST_SEL_MASK);
	apex_params->highg_peak_hyst =
		(APEX_CONFIG5_HIGHG_PEAK_TH_HYST_t)(data[3] &
						    APEX_CONFIG5_HIGHG_PEAK_TH_HYST_SEL_MASK);

	/*
	 * Get params from apex_config9 : ff_debounce_duration, smd_sensitivity and sensitivity_mode
	 */
	apex_params->ff_debounce_duration =
		(APEX_CONFIG9_FF_DEBOUNCE_DURATION_t)(data[4] &
						      APEX_CONFIG9_FF_DEBOUNCE_DURATION_SEL_MASK);
	...
	apex_params->lowg_samples_th =
		(APEX_CONFIG10_LOWG_TIME_TH_SAMPLES_t)(data[5] &
						       APEX_CONFIG10_LOWG_TIME_TH_SEL_MASK);
	...
	apex_params->highg_samples_th =
		(APEX_CONFIG11_HIGHG_TIME_TH_SAMPLES_t)(data[6] &
							APEX_CONFIG11_HIGHG_TIME_TH_SEL_MASK);
