...
} pwm_nxp_flexio_channel_config_type;

struct pwm_nxp_flexio_pulse_info {
	uint8_t pwm_pulse_channels;
	pwm_nxp_flexio_channel_config_type *pwm_info;
};
...
struct pwm_nxp_flexio_config {
	...
	struct pwm_nxp_flexio_pulse_info *pulse_info;
...
static void pwm_nxp_flexio_set_pinlevel(FLEXIO_Type *base, uint8_t pin, bool level)
{
	base->PINOUTD =
...
static bool pwm_nxp_flexio_get_pinoverride(const FLEXIO_Type *const base, uint8_t pin)
{
	return ((base->PINOUTE & (uint32_t)((uint32_t)1U << pin)) != 0UL);
}

static void pwm_nxp_flexio_config_pinoverride(FLEXIO_Type *base, uint8_t pin, bool enabled)
{
	base->PINOUTE =
...
static int pwm_nxp_flexio_get_prescaler_div(const struct device *dev, uint32_t channel)
{
				       ...
				       uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	if (pwm_nxp_flexio_get_pinoverride(flexio_base, pwm_info->pin_id)) {
		pwm_nxp_flexio_config_pinoverride(flexio_base, pwm_info->pin_id, false);
	}
						...
						uint64_t *cycles)
{
	...
	pwm_info = &config->pulse_info->pwm_info[channel];
	prescaler_div = pwm_nxp_flexio_get_prescaler_div(dev, channel);
...
static int mcux_flexio_pwm_init(const struct device *dev)
{
	...
	for (ch_id = 0; ch_id < config->pulse_info->pwm_pulse_channels; ch_id++) {
		...
		pwm_nxp_flexio_set_pinlevel(flexio_base, pwm_info->pin_id, false);
		pwm_nxp_flexio_config_pinoverride(flexio_base, pwm_info->pin_id, false);
		...
		if (pwm_info->polarity == FLEXIO_PWM_ACTIVE_HIGH) {
			...
			/* Timer output is logic zero and is not affected by timer reset */
			timerConfig.timerOutput = kFLEXIO_TimerOutputZeroNotAffectedByReset;
			/* Set the timer mode to dual 8-bit counter PWM low */
			timerConfig.timerMode = kFLEXIO_TimerModeDual8BitPWMLow;
		}
...
#define FLEXIO_PWM_PULSE_GEN_CONFIG(n)								\
	pwm_nxp_flexio_channel_config_type flexio_pwm_##n##_init[] = {				\
		DT_INST_FOREACH_CHILD_STATUS_OKAY(n, _FLEXIO_PWM_PULSE_GEN_CONFIG)		\
	};											\
	const struct pwm_nxp_flexio_pulse_info flexio_pwm_##n##_info = {			\
		.pwm_pulse_channels = ARRAY_SIZE(flexio_pwm_##n##_init),			\
		.pwm_info = (pwm_nxp_flexio_channel_config_type *)flexio_pwm_##n##_init,	\
	};
...
#define FLEXIO_PWM_PULSE_GEN_GET_CONFIG(n)							\
	.pulse_info = (struct pwm_nxp_flexio_pulse_info *)&flexio_pwm_##n##_info,
