...
enum pwm_mcux_flexio_polarity {
	FLEXIO_PWM_ACTIVE_HIGH   = 0x0U,
	FLEXIO_PWM_ACTIVE_LOW    = 0x1U
};

enum pwm_mcux_flexio_timerinit {
...
enum pwm_mcux_flexio_prescaler {
...
enum pwm_mcux_flexio_timer_mode {
...
enum pwm_mcux_flexio_timer_pin {
...
typedef struct {
	...
	enum pwm_mcux_flexio_prescaler prescaler;
	/** Pwm period in ticks */
	uint16_t period;
	/** Pwm duty cycle in ticks */
	uint16_t duty_cycle;
	/** Pwm output polarity */
	enum pwm_mcux_flexio_polarity polarity;
} pwm_mcux_flexio_channel_config_type;

struct pwm_mcux_flexio_pulse_info {
	uint8_t pwm_pulse_channels;
	pwm_mcux_flexio_channel_config_type *pwm_info;
};
...
struct pwm_mcux_flexio_config {
	...
	struct pwm_mcux_flexio_pulse_info *pulse_info;
...
static void pwm_mcux_flexio_set_pinlevel(FLEXIO_Type *base, uint8_t pin, bool level)
{
	base->PINOUTD =
...
static bool pwm_mcux_flexio_get_pinoverride(const FLEXIO_Type *const base, uint8_t pin)
{
	return ((base->PINOUTE & (uint32_t)((uint32_t)1U << pin)) != 0UL);
}

static void pwm_mcux_flexio_config_pinoverride(FLEXIO_Type *base, uint8_t pin, bool enabled)
{
	base->PINOUTE =
...
static int pwm_mcux_flexio_get_prescaler_div(const struct device *dev, uint32_t channel)
{
...
static int pwm_mcux_flexio_set_cycles(const struct device *dev,
				       ...
				       uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	if (pwm_mcux_flexio_get_pinoverride(flexio_base, pwm_info->pin_id)) {
		pwm_mcux_flexio_config_pinoverride(flexio_base, pwm_info->pin_id, false);
	}

	return 0;
}

static int pwm_mcux_flexio_get_cycles_per_sec(const struct device *dev,
						uint32_t channel,
						uint64_t *cycles)
{
	int prescaler_div = 1;
	...
	pwm_info = &config->pulse_info->pwm_info[channel];
	prescaler_div = pwm_mcux_flexio_get_prescaler_div(dev, channel);
...
static int mcux_flexio_pwm_init(const struct device *dev)
{
	...
	for (ch_id = 0; ch_id < config->pulse_info->pwm_pulse_channels; ch_id++) {
		...
		pwm_mcux_flexio_set_pinlevel(flexio_base, pwm_info->pin_id, false);
		pwm_mcux_flexio_config_pinoverride(flexio_base, pwm_info->pin_id, false);
...
static const struct pwm_driver_api pwm_mcux_flexio_driver_api = {
	.set_cycles = pwm_mcux_flexio_set_cycles,
	.get_cycles_per_sec = pwm_mcux_flexio_get_cycles_per_sec,
};
...
#define FLEXIO_PWM_PULSE_GEN_CONFIG(n)								\
	pwm_mcux_flexio_channel_config_type flexio_pwm_##n##_init[] = {				\
		DT_INST_FOREACH_CHILD_STATUS_OKAY(n, _FLEXIO_PWM_PULSE_GEN_CONFIG)		\
	};											\
	const struct pwm_mcux_flexio_pulse_info flexio_pwm_##n##_info = {			\
		.pwm_pulse_channels = ARRAY_SIZE(flexio_pwm_##n##_init),			\
		.pwm_info = (pwm_mcux_flexio_channel_config_type *)flexio_pwm_##n##_init,	\
	};

#define FLEXIO_PWM_TIMER_INDEX_INIT(n)								\
	uint8_t flexio_pwm_##n##_timer_index[ARRAY_SIZE(flexio_pwm_##n##_init)];
...
#define FLEXIO_PWM_PULSE_GEN_GET_CONFIG(n)							\
	.pulse_info = (struct pwm_mcux_flexio_pulse_info *)&flexio_pwm_##n##_info,
