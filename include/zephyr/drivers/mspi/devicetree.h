...
#define MSPI_DEVICE_CONFIG_DT(mspi_dev)                                                           \
	{                                                                                         \
		.ui32CENum            = DT_PROP_OR(mspi_dev, mspi_hardware_ce_num, 0),            \
		.ui32Freq             = DT_PROP(mspi_dev, mspi_max_frequency),                    \
		.eIOMode              = DT_ENUM_IDX_OR(mspi_dev, mspi_io_mode,                    \
						MSPI_IO_MODE_SINGLE),                             \
		.eDataRate            = DT_ENUM_IDX_OR(mspi_dev, mspi_data_rate,                  \
						MSPI_SINGLE_DATA_RATE),                           \
		.eCPP                 = DT_ENUM_IDX_OR(mspi_dev, mspi_cpp_mode, MSPI_CPP_MODE_0), \
		.eEndian              = DT_ENUM_IDX_OR(mspi_dev, mspi_endian,                     \
						MSPI_XFER_LITTLE_ENDIAN),                         \
		.eCEPolarity          = DT_ENUM_IDX_OR(mspi_dev, mspi_ce_polarity,                \
						MSPI_CE_ACTIVE_LOW),                              \
		.bDQSEnable           = DT_PROP(mspi_dev, mspi_dqs_enable),                       \
		.ui32RXDummy          = DT_PROP_OR(mspi_dev, rx_dummy, 0),                        \
		.ui32TXDummy          = DT_PROP_OR(mspi_dev, tx_dummy, 0),                        \
		.ui32ReadInstr        = DT_PROP_OR(mspi_dev, read_instruction, 0),                \
		.ui32WriteInstr       = DT_PROP_OR(mspi_dev, write_instruction, 0),               \
		.ui16InstrLength      = DT_ENUM_IDX_OR(mspi_dev, instruction_length, 0),          \
		.ui16AddrLength       = DT_ENUM_IDX_OR(mspi_dev, address_length, 0),              \
		.ui32MemBoundary      = COND_CODE_1(DT_NODE_HAS_PROP(mspi_dev, ce_break_config),  \
						(DT_PROP_BY_IDX(mspi_dev, ce_break_config, 0)),   \
						(0)),                                             \
		.ui32BreakTimeLimit   = COND_CODE_1(DT_NODE_HAS_PROP(mspi_dev, ce_break_config),  \
...
#define MSPI_XIP_CONFIG_DT_NO_CHECK(mspi_dev)                                                     \
	{                                                                                         \
		.ui32AddrOffset       = DT_PROP_BY_IDX(mspi_dev, xip_config, 1),                  \
		.ePermission          = DT_PROP_BY_IDX(mspi_dev, xip_config, 3),                  \
		.ui32Size             = DT_PROP_BY_IDX(mspi_dev, xip_config, 2),                  \
		.bEnable              = DT_PROP_BY_IDX(mspi_dev, xip_config, 0),                  \
	}
...
#define MSPI_SCRAMBLE_CONFIG_DT_NO_CHECK(mspi_dev)                                                \
	{                                                                                         \
		.bEnable              = DT_PROP_BY_IDX(mspi_dev, scramble_config, 0),             \
		.ui32AddrOffset       = DT_PROP_BY_IDX(mspi_dev, scramble_config, 1),             \
		.ui32Size             = DT_PROP_BY_IDX(mspi_dev, scramble_config, 2),             \
	}
