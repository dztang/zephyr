 ...
 */
typedef int (*led_api_length)(const struct device *dev);
 ...
 * @param dev	LED strip device.
 *
 * @retval	Positive length of LED strip device on success.
 * @retval	-errno negative errno code on failure.
 */
static inline int led_strip_length(const struct device *dev)
{
	const struct led_strip_driver_api *api =
		(const struct led_strip_driver_api *)dev->api;
