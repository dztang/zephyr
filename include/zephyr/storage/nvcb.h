 ...
 *
 * (binary) string-type:
 *    - 0b01000000 - 0b01010111 followed by the payload,
 *    - 0b01011000 followed by 1 byte (payload length) followed by the payload,
 ...
 *
 */
#define DEFINE_NVCB(_name, _ctx, _pbuf, _psz, _bsz, _bcnt, _bspr, _read, _prog, \
		    _prep, _sync, _init, _release, _lock, _unlock)              \
...
/**
 * @brief walk forward over spare blocks of the nvcb and for each entry in a
 * block (forward stepping) issue a callback. Walking is stopped when the
 * callback returns NVCB_DONE or an errorcode. Walking skips to the next block
 * when the callback returns NVCB_SKIP.
 *
 * @param[in] store nvcb store
 * @param[in] cb callback routine
 * @param[in] cb_arg callback routine argument
...
/**
 * @brief walk reverse over spare blocks of the nvcb and for each entry in a
 * block (forward stepping) issue a callback. Walking is stopped when the
 * callback returns NVCB_DONE or an errorcode. Walking skips to the previous
 * block when the callback returns NVCB_SKIP.
 *
 * @param[in] store nvcb store
 * @param[in] cb callback routine
 * @param[in] cb_arg callback routine argument
