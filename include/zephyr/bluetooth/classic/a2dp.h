...
{\
	.codec_type = _codec,\
	.seid = {.sep = {.media_type = BT_AVDTP_AUDIO, .tsep = _role}},\
...
struct bt_a2dp_ep_info {
	...
	struct bt_avdtp_seid_info seid_info;
};
...
/**
 * @brief Callback function for bt_a2dp_discover()
 *
 * Called when a stream endpoint is discovered
 *
 * @param a2dp      a2dp connection.
 * @param info      found endpoint info
 * @param ep  return the found endpoint.
 */
...
struct bt_a2dp_discover_param {
	 ...
	 */
	struct bt_avdtp_seid_info *seids;
	/** count of seids */
	uint8_t seid_count;
};
