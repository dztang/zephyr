...
#define NVCB_CBORHDR0B  0x40   /* cbor byte string 0 byte length */
#define NVCB_MAXLEN0B   0x17   /* max length 0 byte length cbor byte string */
#define NVCB_CBORHDR1B  0x58   /* cbor byte string 1 byte length */
#define NVCB_MAXLEN1B   0xFF   /* max length 1 byte length cbor byte string */
#define NVCB_CBORHDR2B  0x59   /* cbor byte string 2 byte length */
#define NVCB_MAXLEN2B   0xFFFF /* max length 2 byte length cbor byte string */
#define NVCB_BREAKCHAR  0xFF   /* cbor atom break */
#define NVCB_EEHDRSIZE  4      /* extended entry header size */
#define NVCB_BUFSIZE    64
#define NVCB_INITPASS   0xF0   /* initial pass value */
...
static size_t make_header(uint8_t *hdr, size_t dlen, uint8_t pass)
{
	...
	if (dlen > NVCB_MAXLEN1B) {
		hdr[0] = NVCB_CBORHDR2B;
		hdr[1] = dlen >> 8;
		hdr[2] = dlen;
		hdr[3] = pass;
		rv = 4U;
	...
	if (dlen > NVCB_MAXLEN0B) {
		hdr[0] = NVCB_CBORHDR1B;
		hdr[1] = dlen;
		hdr[2] = pass;
		rv = 3U;
		goto end;
...
static int header_get_prop(const uint8_t *hdr, size_t *hsz, size_t *dsz)
{
	...
	if ((hdr[0] > NVCB_CBORHDR2B) || (hdr[0] < NVCB_CBORHDR0B)) {
		goto end;
	}

	rc = 0;
	if (hdr[0] == NVCB_CBORHDR2B) {
		(*hsz) = 4U;
		(*dsz) = (hdr[1] << 8) + hdr[2];
		goto end;
	}

	if (hdr[0] == NVCB_CBORHDR1B) {
		(*hsz) = 3U;
		(*dsz) = hdr[1];
		goto end;
	}
...
static bool header_pass_match(const uint8_t *hdr, uint8_t pass)
{
	...
	if ((hdr[0] == NVCB_CBORHDR2B) && (hdr[3] == pass)) {
		rv = true;
		goto end;
	}

	if ((hdr[0] == NVCB_CBORHDR1B) && (hdr[2] == pass)) {
		rv = true;
		goto end;
	}

	if (((hdr[0] & NVCB_CBORHDR0B) == NVCB_CBORHDR0B) && (hdr[1] == pass)) {
		rv = true;
		goto end;
	}
...
static void walk_forward(const struct nvcb_store *store,
			 const struct nvcb_bctx *bctx,
			 int (*cb)(const struct nvcb_ent *ent, void *cb_arg),
			 void *cb_arg)
{
	struct nvcb_ent ent;
...
static void walk_reverse(const struct nvcb_store *store,
			 const struct nvcb_bctx *bctx,
			 int (*cb)(const struct nvcb_ent *ent, void *cb_arg),
			 void *cb_arg)
{
...
static int init_nvcb_store_data(const struct nvcb_store *store)
{
	...
	walk_forward(store, &bctx, init_cblck_cb, (void *)&found);
	if (!found) {
		data->cblck = 0U;
		data->cpos = 0U;
		goto end;
	...
	bctx.bcnt = 1;
	walk_reverse(store, &bctx, init_cpos_cb, (void *)&cpos);
	data->cpos = cpos;
end:
	unlock(store);
	return rc;
...
int nvcb_append(const struct nvcb_store *store, size_t len, struct nvcb_ent *ent)
{
	...
	if (len > NVCB_MAXLEN2B) {
		goto end;
	}

	if ((len > NVCB_MAXLEN1B) && (store->cfg->bsz < (len + 4U))) {
		goto end;
	}

	if ((len > NVCB_MAXLEN0B) && (store->cfg->bsz < (len + 3U))) {
		goto end;
	}

	if (store->cfg->bsz < (len + 2U)) {
		goto end;
	}
		      ...
		      void *cb_arg)
{
	...
	walk_forward(store, &bctx, cb, cb_arg);
	rc = 0;
end:
	return rc;
}

int nvcb_spare_walk_forward(const struct nvcb_store *store,
			    int (*cb)(const struct nvcb_ent *ent, void *cb_arg),
			    void *cb_arg)
{
	int rc = NVCB_EINVAL;
		      ...
		      void *cb_arg)
{
	...
	const struct nvcb_bctx bctx = {
		.skip = 0U,
		.bcnt = store->cfg->bcnt - store->cfg->bspr,
	};

	walk_reverse(store, &bctx, cb, cb_arg);
	rc = 0;
end:
	return rc;
}

int nvcb_spare_walk_reverse(const struct nvcb_store *store,
			    int (*cb)(const struct nvcb_ent *ent, void *cb_arg),
			    void *cb_arg)
{
	int rc = NVCB_EINVAL;
	...
	const struct nvcb_bctx bctx = {
		.skip = store->cfg->bcnt - store->cfg->bspr,
		.bcnt = store->cfg->bcnt,
	};
