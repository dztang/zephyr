...
struct bt_avdtp_ops_cb {
	...
	int (*get_capabilities_ind)(struct bt_avdtp *session,
		struct bt_avdtp_seid *seid, struct net_buf *rsp_buf, uint8_t *errcode);

	int (*set_configuration_ind)(struct bt_avdtp *session, struct bt_avdtp_seid *seid,
		uint8_t int_seid, struct net_buf *buf, uint8_t *errcode);

	int (*open_ind)(struct bt_avdtp *session,
		struct bt_avdtp_seid *seid, uint8_t *errcode);
...
struct bt_avdtp {
	...
	struct bt_avdtp_seid *current_seid;
