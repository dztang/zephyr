...
static int a2dp_get_capabilities_ind(struct bt_avdtp *session, struct bt_avdtp_seid *seid,
		struct net_buf *rsp_buf, uint8_t *errcode)
{
	...
	*errcode = 0;
	...
	ep = CONTAINER_OF(seid, struct bt_a2dp_ep, seid);
	/* Length Of Service Capability */
	net_buf_add_u8(rsp_buf, ep->codec_cap->len + 2u);
	/* Media Type */
	net_buf_add_u8(rsp_buf, seid->sep.media_type << 4u);
	/* Media Codec Type */
...
static int a2dp_set_config_ind(struct bt_avdtp *session, struct bt_avdtp_seid *seid,
...
static int a2dp_open_ind(struct bt_avdtp *session, struct bt_avdtp_seid *seid, uint8_t *errcode)
{
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
	struct bt_a2dp_stream_ops *ops;
...
static int a2dp_start_ind(struct bt_avdtp *session, struct bt_avdtp_seid *seid, uint8_t *errcode)
{
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
	struct bt_a2dp_stream_ops *ops;
...
static int a2dp_close_ind(struct bt_avdtp *session, struct bt_avdtp_seid *seid, uint8_t *errcode)
{
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
	struct bt_a2dp_stream_ops *ops;
...
static int a2dp_suspend_ind(struct bt_avdtp *session, struct bt_avdtp_seid *seid, uint8_t *errcode)
{
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
	struct bt_a2dp_stream_ops *ops;
...
static int bt_a2dp_open_cb(struct bt_avdtp_req *req)
{
	...
	ep = CONTAINER_OF(a2dp->open_param.seid, struct bt_a2dp_ep, seid);
	if ((ep == NULL) || (ep->stream == NULL)) {
		return -1;
	}
	stream = ep->stream;
...
static int bt_a2dp_set_config_cb(struct bt_avdtp_req *req)
{
	...
	ep = CONTAINER_OF(a2dp->set_config_param.seid, struct bt_a2dp_ep, seid);
	if ((ep == NULL) || (ep->stream == NULL)) {
		return -1;
	}
	stream = ep->stream;
...
static int bt_a2dp_get_capabilities_cb(struct bt_avdtp_req *req)
{
	...
	if (a2dp->get_capabilities_param.status) {
		...
		(a2dp->discover_cb_param->cb != NULL)) {
			...
			a2dp->discover_cb_param->cb(a2dp, NULL, &ep);
	...
	if ((a2dp->discover_cb_param != NULL) && (a2dp->discover_cb_param->cb != NULL)) {
		...
		info->seid_info = a2dp->discover_cb_param->seids[a2dp->get_cap_index];
...
static int bt_a2dp_get_seid_caps(struct bt_a2dp *a2dp)
{
	...
	for (; a2dp->get_cap_index < a2dp->peer_seids_count;
			a2dp->get_cap_index++) {
		if (a2dp->discover_cb_param->seids[a2dp->get_cap_index].media_type ==
			BT_AVDTP_AUDIO) {
			...
			a2dp->get_capabilities_param.stream_endpoint_id =
				a2dp->discover_cb_param->seids[a2dp->get_cap_index].id;
...
static int bt_a2dp_discover_cb(struct bt_avdtp_req *req)
{
	...
	if (!(DISCOVER_REQ(req)->status)) {
		...
		while ((seid != NULL) &&
			(a2dp->peer_seids_count < a2dp->discover_cb_param->seid_count)) {
			LOG_DBG("id:%d, inuse:%d, media_type:%d, tsep:%d, ",
				seid->id,
				seid->inuse,
				seid->media_type,
				seid->tsep);
			a2dp->discover_cb_param->seids[a2dp->peer_seids_count] = *seid;
...
static int bt_a2dp_start_cb(struct bt_avdtp_req *req)
{
	...
	ep = CONTAINER_OF(a2dp->start_param.seid, struct bt_a2dp_ep, seid);
	if ((ep == NULL) || (ep->stream == NULL)) {
		return -1;
	}
	stream = ep->stream;
...
int bt_a2dp_discover(struct bt_a2dp *a2dp, struct bt_a2dp_discover_param *param)
{
	...
	return bt_avdtp_discover(&a2dp->session, &a2dp->discover_param);
}
		...
		struct bt_a2dp_codec_cfg *config)
{
	...
	stream->remote_ep_id = remote_ep->seid.sep.id;
	stream->a2dp = a2dp;
	local_ep->stream = stream;
	remote_ep->stream = stream;
	a2dp->set_config_param.req.func = bt_a2dp_set_config_cb;
	a2dp->set_config_param.acp_stream_ep_id = remote_ep->seid.sep.id;
	a2dp->set_config_param.int_stream_endpoint_id = local_ep->seid.sep.id;
	...
	a2dp->set_config_param.seid = &local_ep->seid;
...
int bt_a2dp_stream_establish(struct bt_a2dp_stream *stream)
{
	...
	a2dp->open_param.acp_stream_ep_id = stream->remote_ep == NULL ?
		stream->remote_ep->seid.sep.id : stream->remote_ep_id;
	a2dp->open_param.seid = a2dp->set_config_param.seid;
...
int bt_a2dp_stream_start(struct bt_a2dp_stream *stream)
{
	...
	a2dp->start_param.acp_stream_ep_id = stream->remote_ep == NULL ?
		stream->remote_ep->seid.sep.id : stream->remote_ep_id;
	a2dp->start_param.seid = &stream->local_ep->seid;
...
uint32_t bt_a2dp_get_mtu(struct bt_a2dp_stream *stream)
{
	...
	return bt_avdtp_get_media_mtu(&stream->local_ep->seid);
}
			...
			uint16_t seq_num, uint32_t ts)
{
	...
	/* send the buf */
	return bt_avdtp_send_media_data(&stream->local_ep->seid, buf);
}
#endif
...
int bt_a2dp_register_ep(struct bt_a2dp_ep *ep, uint8_t media_type, uint8_t role)
{
	...
	if (role == BT_AVDTP_SINK) {
		ep->seid.media_data_cb = bt_a2dp_media_data_callback;
	} else {
		ep->seid.media_data_cb = NULL;
	}
#else
	ep->seid.media_data_cb = NULL;
#endif
	A2DP_LOCK();
	err = bt_avdtp_register_sep(media_type, role, &(ep->seid));
	A2DP_UNLOCK();
	if (err < 0) {
		return err;
	}
