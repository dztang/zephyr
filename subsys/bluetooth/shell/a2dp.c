...
static void shell_a2dp_print_capabilities(struct bt_a2dp_ep_info *ep_info)
{
	...
	shell_print(ctx_shell, "endpoint id: %d, %s, %s:", ep_info->seid_info.id,
			(ep_info->seid_info.tsep == BT_AVDTP_SINK) ? "(sink)" : "(source)",
			(ep_info->seid_info.inuse) ? "(in use)" : "(idle)");
	...
	*rsp_err_code = 0;
	...
	if (*rsp_err_code == 0) {
		...
		sample_rate  = bt_a2dp_sbc_get_sampling_frequency(
					&codec_cfg->codec_config->codec_ie[0]);
...
static struct bt_avdtp_seid_info found_seids[5];
