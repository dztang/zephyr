...
#if defined(CONFIG_BT_CAP_BROADCAST_ASSISTANT)
static void cap_broadcast_reception_start_cb(struct bt_conn *conn, int err)
{
	if (err != 0) {
		shell_error(ctx_shell, "Broadcap reception start failed (%d)", err);
		return;
	}
	shell_print(ctx_shell, "Broadcap reception start completed");
}
#endif
						       ...
						       char *argv[])
{
	...
	member_param->subgroups = &subgroup;
	...
	shell_print(sh, "Starting broadcast on %zu connections", param.count);
