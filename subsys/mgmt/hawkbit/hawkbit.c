...
static void s_probe(void *o)
{
	...
	if (s->hb_context.code_status == HAWKBIT_METADATA_ERROR ||
	    s->hb_context.code_status == HAWKBIT_PERMISSION_ERROR) {
		smf_set_state(SMF_CTX(s), &hawkbit_states[S_HAWKBIT_TERMINATE]);
		return;
	}
...
static void s_hawbit_close(void *o)
{
	...
	if (url_buffer == NULL) {
		LOG_ERR("Can't find %s url: %d", "cancelAction");
...
static void s_download(void *o)
{
	...
	if (s->hb_context.code_status == HAWKBIT_DOWNLOAD_ERROR) {
		smf_set_state(SMF_CTX(s), &hawkbit_states[S_HAWKBIT_TERMINATE]);
		return;
	}
