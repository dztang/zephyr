...
if [ "${BOARD}" == "nrf5340bsim_nrf5340_cpuapp" ]; then
  ...
  app=samples/bluetooth/broadcast_audio_source \
    ...
    exe_name=bs_${BOARD}_${app}_prj_conf_overlay-sequential_conf sysbuild=1 compile
  app=samples/bluetooth/broadcast_audio_source \
    conf_overlay='overlay-bt_ll_sw_split.conf;overlay-interleaved.conf' \
    exe_name=bs_${BOARD}_${app}_prj_conf_overlay-interleaved_conf sysbuild=1 compile
