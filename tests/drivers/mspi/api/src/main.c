...
static const struct device *mspi_devices[] = {
	DT_FOREACH_CHILD_STATUS_OKAY(MSPI_BUS_NODE, DEVICE_DT_GET),
};
...
struct mspi_cfg hardware_cfg = {
	...
	.ui32MaxFreq              = DT_PROP_OR(MSPI_BUS_NODE, clock_frequency, 48000000),
	.bReinit                  = true,
};
#endif
...
static struct mspi_dev_id dev_id[] = {
	DT_FOREACH_CHILD_STATUS_OKAY(MSPI_BUS_NODE, MSPI_DEVICE_ID_DT),
};

static struct mspi_dev_cfg device_cfg[] = {
	DT_FOREACH_CHILD_STATUS_OKAY(MSPI_BUS_NODE, MSPI_DEVICE_CONFIG_DT),
};

static struct mspi_xip_cfg xip_cfg[] = {
	DT_FOREACH_CHILD_STATUS_OKAY(MSPI_BUS_NODE, MSPI_XIP_CONFIG_DT),
};

static struct mspi_scramble_cfg scramble_cfg[] = {
	DT_FOREACH_CHILD_STATUS_OKAY(MSPI_BUS_NODE, MSPI_SCRAMBLE_CONFIG_DT),
};

static mspi_timing_cfg timing_cfg;
static mspi_timing_param timing_cfg_mask = 0;
